...
/**
 * Prometheus to Openmetrics (and PAGODA) YANGTools Meta Driver.
 */
@Tags({"Prometheus to Openmetrics (and PAGODA) YANGTools Meta Driver"})
@CapabilityDescription("NiFi processor to implement the Prometheus to Openmetrics (and PAGODA) YANGTools Meta Driver.")
@SeeAlso({})
@ReadsAttributes({@ReadsAttribute(attribute="", description="")})
@WritesAttributes({@WritesAttribute(attribute="", description="")})
public class PrometheusToOpenmetricsMetaDriver extends AbstractProcessor {
        ...
        try {    
            session.read(flowfile, new InputStreamCallback() {
                ...
                public void process(InputStream in) throws IOException {
                    try{
                        if(driver_type.equals("PrometheusToOpenmetricsTransformer")){
                            JsonReader reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
                            PrometheusToOpenmetricsTransformerDriver transformer_driver = new PrometheusToOpenmetricsTransformerDriver(metric_family_name, metric_family_type, metric_family_unit, metric_family_help);
                            String json_ietf = transformer_driver.driver(reader);
                            value.set(json_ietf);
                        } else {
                            if(driver_type.equals("MetricSource")){
                                JsonParser jsonParser = new JsonParser();
                                JsonArray jsonArray = (JsonArray) jsonParser.parse(new InputStreamReader(in, "UTF-8"));
                                PrometheusSourceDriver source_driver = new PrometheusSourceDriver();
                                String json_ietf = source_driver.driver(jsonArray);
                                value.set(json_ietf);
                            } else {
                                if(driver_type.equals("PrometheusToPagodaTransformer")){
                                    JsonReader reader = new JsonReader(new InputStreamReader(in, "UTF-8"));
                                    PrometheusToPagodaTransformerDriver pagoda_driver = new PrometheusToPagodaTransformerDriver();
                                    String json_ietf = pagoda_driver.driver(reader);
                                    value.set(json_ietf);
                                }
                            }
                        }
